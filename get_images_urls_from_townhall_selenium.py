...
while chk_date == True:
        ...
        btn_forward = s.find_element(By.XPATH, "//*[@id='slideshow-container']/div[1]/div[1]/div[1]/a/i").click()
        bs_obj = BSoup(s.page_source, 'html.parser')
        time.sleep(1)
        toon_date = s.find_element(By.XPATH, "//*[@id='slideshow-container']/div[1]/div[2]/h4/span")
...
cursor.execute('''
    CREATE TABLE img_links(id INTEGER PRIMARY KEY, link TEXT, name TEXT, content TEXT)
''')
db.commit()

for i in range(0, len(toons_lnks)):
    cursor.execute('''INSERT INTO img_links (id, link, name, content) VALUES (?,?,?,?)''', (i+1, toons_lnks[i], authors[i], dates[i]))
